...
bool ModulePhysics::Start()
{
	...
	b2Vec2 *aux = new b2Vec2[19];
	for (uint i = 0; i < 19; ++i) {

		aux[i].x = PIXEL_TO_METERS(points[i*2]);
		aux[i].y = PIXEL_TO_METERS(points[i*2 + 1]);
	}
	...
	b2Body* curve = world->CreateBody(&body);
	b2ChainShape shape;
	CreateChain(PIXEL_TO_METERS(x + 200), PIXEL_TO_METERS(y - 500), points, 38, body.type);
	shape.CreateLoop(aux, 19);

	b2FixtureDef fixture;
	fixture.shape = &shape;
	curve->CreateFixture(&fixture);
...
PhysBody* ModulePhysics::CreateChain(int x, int y, int* points, int size, b2BodyType type, bool ARRIBA_ESPANA) //bool ARRIBA_ESPAÑA = closing or not closing the shape
{
	b2BodyDef body;
	body.type = type;
	body.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
